...
module.exports = {
  config: {
    ...
    guide: {
      vi:
        "   {pn} [add | -a] <uid | @tag>: Thêm quyền author cho người dùng" +
        "\n	  {pn} [remove | -r] <uid | @tag>: Xóa quyền author của người dùng" +
        "\n	  {pn} [list | -l]: Liệt kê danh sách author",
      en:
        "   {pn} [add | -a] <uid | @tag>: Add author role for user" +
  ...
  langs: {
    vi: {
      ...
      missingIdAdd:
        "⚠️ | Vui lòng nhập ID hoặc tag người dùng muốn thêm quyền author",
      removed: "✅ | Đã xóa quyền author của %1 người dùng:\n%2",
      notAdmin: "⚠️ | %1 người dùng không có quyền author:\n%2",
      missingIdRemove:
        "⚠️ | Vui lòng nhập ID hoặc tag người dùng muốn xóa quyền author",
      ...
      case "-": {
        if (args[1]) {
          ...
          for (const uid of uids) {
            if (config.GOD.includes(uid)) authorIds.push(uid);
