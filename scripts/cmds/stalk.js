...
return api.sendMessage(
    "⚠ |  I couldn't find the user ID from the provided link. Please try again with the user ID.\n\nExample ➾ .stalk 100073291639820",
    event.threadID
);
    }
    if (linkUid) {
...
userName = user[userId].name;
  ...
  const response = await axios.get(`${global.GoatBot.config.API.DIPTO}/fbinfo?id=${userId}&key=dipto008`);
