...
module.exports = {
  ...
  onStart: async function ({ args, event, message, getLang }) {
    ...
    let msgSend = null;
    try {
      const response = await axios.get(`https://hazee-downloader.onrender.com/twitter?url=${args[0]}`);
      ...
      const stream = await global.utils.getStreamFromURL(response.data.result.mediaURLs[0]);

      await message.reply({ body: `Downloaded Twitter Video <😶`,
        attachment: stream });
